<template>
  <v-card flat :loading="isFetching">
    <v-sheet class="d-block d-sm-flex justify-space-between mb-3">
      <v-sheet class="" width="720">
        <v-row no-gutters>
          <v-col cols="4" class="font-weight-bold">
            <h6 class="text-h6">Province:</h6>
          </v-col>
          <v-col cols="8">
            <h6 class="text-h6">
              {{ provinceName }}
                ...
                <tr :key="'location-' + i">
                  <td>
                    <a
                      ...
                      href="#"
                      @click.prevent="$emit('municipality:click', form)">
                      {{ form.municipality }}
                    </a>
                  </td>
                  <td>
          ...
          <v-data-table
            ...
            <template #item.municipality="{ item }">
              <a
                class="text-decoration-none"
                href="#"
                @click.prevent="$emit('municipality:click', item)">
                {{ item.municipality }}
              </a>
            </template>
...
export default {
    ...
    filteredForms() {
      if (this.search) {
        return this.forms.filter(
          record =>
            record.municipality.search(new RegExp(this.search, 'i')) !== -1,
        );
      }
      return this.forms;
    },
  ...
  methods: {
    async fetchData() {
      ...
      try {
        const { data } = await this.$axios.get(
          `/api/qas-form/report/scorecard/province?regioncode=${this.regionCode}&provincecode=${this.provinceCode}`,
        );
    ...
    onExportCSV() {
      ...
      link.setAttribute('download', 'provincial-scorecard.csv');
    ...
    onExportPDF() {
      ...
      doc.setFontSize(14);
      doc.text('PROVINCE SCORECARD (BETA)', leftMargin, 16);
      doc.text('BDRRMP QUALITY ASSESSMENT SCORECARD', leftMargin, 24);
      doc.text(`Province: ${this.provinceName}`, leftMargin, 30);
      doc.setFontSize(10);
      doc.text(`Region: ${this.regionName}`, leftMargin, 35);
      doc.text(
      ...
      doc.autoTable({
        theme: 'grid',
        columns: [
          { header: 'City/Municipality', dataKey: 'municipality' },
